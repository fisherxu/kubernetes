...
func GetAllReplicaSets(deployment *apps.Deployment, c appsv1.AppsV1Interface) ([]*apps.ReplicaSet, []*apps.ReplicaSet, *apps.ReplicaSet, error) {
...
// Note that the first set of old replica sets doesn't include the ones with no pods, and the second set of old replica sets include all old replica sets.
func GetOldReplicaSets(deployment *apps.Deployment, c appsv1.AppsV1Interface) ([]*apps.ReplicaSet, []*apps.ReplicaSet, error) {
	rsList, err := ListReplicaSets(deployment, RsListFromClient(c))
	if err != nil {
		return nil, nil, err
	}
...
func GetNewReplicaSet(deployment *apps.Deployment, c appsv1.AppsV1Interface) (*apps.ReplicaSet, error) {
	rsList, err := ListReplicaSets(deployment, RsListFromClient(c))
	if err != nil {
		return nil, err
	}
...
func RsListFromClient(c appsv1.AppsV1Interface) RsListFunc {
...
func WaitForReplicaSetUpdated(c appslisters.ReplicaSetLister, desiredGeneration int64, namespace, name string) error {
...
func WaitForPodsHashPopulated(c appslisters.ReplicaSetLister, desiredGeneration int64, namespace, name string) error {
	return wait.PollImmediate(1*time.Second, 1*time.Minute, func() (bool, error) {
		rs, err := c.ReplicaSets(namespace).Get(name)
		if err != nil {
			return false, err
...
func LabelPodsWithHash(podList *v1.PodList, c clientset.Interface, podLister corelisters.PodLister, namespace, name, hash string) error {
	for _, pod := range podList.Items {
		...
		if pod.Labels[apps.DefaultDeploymentUniqueLabelKey] != hash {
			_, err := UpdatePodWithRetries(c.CoreV1().Pods(namespace), podLister, pod.Namespace, pod.Name,
				func(podToUpdate *v1.Pod) error {
					...
					if podToUpdate.Labels[apps.DefaultDeploymentUniqueLabelKey] == hash {
						return errors.ErrPreconditionViolated
					}
					podToUpdate.Labels = labelsutil.AddLabel(podToUpdate.Labels, apps.DefaultDeploymentUniqueLabelKey, hash)
					return nil
				})
			if err != nil {
